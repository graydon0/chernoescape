...
{	
	...
	if (_follow_cam == 0) then {NORRN_REVIVE_cam switchCamera "INTERNAL"} else {NORRN_REVIVE_cam switchCamera "EXTERNAL"};
	NORRN_REVIVE_cam CameraEffect ["Terminate","Back"];
	...
	if (_follow_cam == 1) then
	{
		...
		//create dialog and initialise keyboard;
		_r_dialog_0 = createDialog "rev_cam_dialog_blank"; 
		_r_display = findDisplay 99993;
		_r_display displaySetEventHandler ["Keydown", "_this call CAM_KEY_pressed"];
		CAM_KEY_pressed = compile preprocessfile "revive_sqf\CAM_KEY_pressed.sqf";
	};
...
while {!_destroy} do 
{
	    ...
	    time >= _show_respawn_time && !NORRN_r_time_expire && _dialog_fade != Norrn_DialogBlink || _nearest_teammate_dialog == 1 && _nearest_friend == 1 && !NORRN_r_time_expire && _dialog_fade != Norrn_DialogBlink) then 
	{
  	...
  	if (NORRN_r_time_expire && _d < 2 || _all_dead_dialog == 1 && all_dead_check && _d < 2 && !NORRN_r_time_expire || NORRN_r_time_expire && _d == 1000 && _respawn_position == 2) then 
	{	
		unconscious_body setVariable ["NORRN_AIunconscious", false, true];
		if (_all_dead_dialog == 1 && all_dead_check && _d < 2 || _d == 1000) then
		{	
			unconscious_body switchMove "Deadstate";
			norrnRDead = unconscious_body;
			publicVariable "norrnRDead";
			if (_unconscious_markers == 1) then {deleteMarker format["%1 is down", player]};
			if (_d == 1000) then
			{
				titleText ["\n\n\n\n\n\n\n\n\n\nRevive timer has been exceeded. Choose a marker for respawn", "BLACK FADED", 10];
			} else {
	...
	if (_follow_cam == 0) then {
		//if (!dialog) then {_r_dialog_0 = createDialog "rev_cam_dialog_blank"}; 
		vehicle unconscious_body switchCamera "INTERNAL"; 
		NORRN_REVIVE_cam CameraEffect ["Terminate","Back"];
		if (_blackOut == 500) then
		{
			titleText ["", "BLACK", 1];
