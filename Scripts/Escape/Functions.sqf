...
drn_fnc_Escape_BuildAmmoDepot = {
    private ["_middlePos", "_staticWeaponClasses", "_parkedVehicleClasses", "_parkedAircraftClasses"];
    private ["_object", "_pos", "_marker", "_instanceNo", "_randomNo", "_gun", "_angle", "_car", "_aircraft"];
    
    _middlePos = _this select 0;
    _staticWeaponClasses = _this select 1;
    _parkedVehicleClasses = _this select 2;
    _parkedAircraftClasses = _this select 3;
    ...
    */
    ...
    if (random 10 > 1 && count _parkedAircraftClasses > 0) then {
        _aircraft = _parkedAircraftClasses select floor random count _parkedAircraftClasses;
    }
    else {
        _aircraft = "";
    };
    ...
    if (_aircraft != "") then {
        _randomNo = random 4;
        _pos = [(_middlePos select 0) + 15, (_middlePos select 1), 0];
        _angle = 45;
        if ((random 100) > 50) then {_angle = 0;} else {_angle = 180;};
        ...
        if (_randomNo > 3) then {
            _pos = [(_middlePos select 0), (_middlePos select 1) + 15, 0];
            if ((random 100) > 50) then {_angle = 90;} else {_angle = 270;};
        };
        
        _object = _aircraft createVehicle _pos;
        _object setPos _pos;
        _object setDir _angle;
    };
...
drn_fnc_Escape_AddRemoveComCenArmor = {
    ...
    if (count _armorObjects == 0) then {
        ...
        
        {
            ...
            _result = [_spawnPos, 0, _x, west] call BIS_fnc_spawnVehicle;
